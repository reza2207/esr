...
class Serah_terima_umum extends CI_Controller {
	...
	{	
		if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
			...
			$data->url = $this->Berkas_model->get_url()->result();
			$this->load->view('header', $data);
			$this->load->view('serah_terima_umum', $data);
		}else{
	...
	{	
		...
		if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
			...
			if($this->input->post(null)){   
				...
				//validasi
				$this->form_validation->set_rules('nama_file', 'Nama File', 'required|min_length[1]');
		        $this->form_validation->set_rules('ext', 'Extention File', 'required');
		        $this->form_validation->set_rules('url', 'Url', 'required');
				$this->form_validation->set_rules('perihal', 'Perihal', 'required|min_length[5]|max_length[250]');
		        ...
		        if ($this->form_validation->run() == FALSE){
					...
					$oldfile = $this->input->post('nama_file').$this->input->post('ext');
					$newname = tgl_indo($tgl).' '.$no.' '.$perihal.$this->input->post('ext');
					$dari = $this->input->post('dari');
					$kepada = $this->input->post('kepada');
					$nama = safename($newname);

                    $created = $_SESSION['nama'];

					if(!file_exists($url."/".$oldfile)){
						$data = new stdClass();
						$data->type = 'error';
						$data->message = "The file $oldfile does not exist";
					}else{
						if($this->Berkas_model->store($nama, $url, $perihal, $tgl, $no, $created, $dari, $kepada))
						{
							rename($url."/".$oldfile, $url."/".safename($newname));
							$data = new stdClass();
							$data->type = 'success';
